...
def synthesize_node(state: TurnState, deps: AgentDeps) -> dict:
    ...
    if capped:
        listed = ", ".join(
            f"{key}: {frame.row_count} rows matched, {len(frame.rows)} shown"
            for key, frame in capped
        )
        results += (
            f"\n\nThese results are SAMPLES — {listed}. The row counts are exact, "
            "so use them when the question asks how many. Do not total or average "
            "the rows shown and do not describe them as all of them: they are "
            "examples, and the list is partial."
        )
    ...
    prompt = SYNTHESIS_PROMPT.format(
        ...
        # How analysts here write a finding. The definitions say what to
        # measure; this says what a good answer looks like — split by cohort,
        # compare against a baseline, close with numbered actions.
        examples=style_examples(recalled(state, deps)),
    )
